...
<script setup>
    ...
    const deleteBrand = async (id) => {
        try {
            const res = await axios.delete(`https://gh-server-83lb.onrender.com/api/delete-brand/${id}`)
            if(res.status === 200) {
                getData()
            }
        } catch(err) {
    ...
    const addBrand = async (e) => {
        ...
        try {
            ...
            console.log(data)
            const res = await axios.post('https://gh-server-83lb.onrender.com/api/add-brand', data, {
                header: {
