<template>
    <div v-if="findProduct">
        ...
        <h2 class="text-2xl mt-16 mb-2"><b>{{ findProduct.product_title }}</b> продукт изменения.</h2>
        <hr class="mb-8"/>
        <div>
            <form @submit="updateProduct">
                <div class="flex flex-wrap justify-between">
                    <div class="w-80">
                        ...
                        <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 mb-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-600" id="productTitle" :value="findProduct.product_title" placeholder="Название продукта" required>
                        ...
                        <div v-else>
                            <button @click="deleteOffer" class="m-0 mt-2 text-center ml-36 mb-1 p-1 rounded bg-blue-600 text-white w-44 cursor-pointer">Отменить Скидку</button>
                        </div>
                        <label class="block text-gray-700 text-lg w-full font-bold mb-2" for="productPrice">Выберите Бренд</label>
                        ...
                        <textarea class="shadow resize-none appearance-none h-40 border rounded w-full py-2 px-3 mb-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-600" id="productDesc" :value="findProduct.product_desc" placeholder="Информация о продукте"></textarea>
                    </div>
                    <div class="w-80">
                        <label class="block text-gray-700 text-lg w-full font-bold m-0" for="productImage">Изображение продукта</label>
                        <p class="block text-gray-700 text-sm w-full font-bold m-0 mb-2">Минимум 2 изображения требуется!</p>
                ...
                <button type="submit" class="float-right bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-600 mt-2">Обновить Продукт</button>
            </form>
        </div>
    </div>
</template>
...
<script setup>
    ...
    onMounted(async() => {
        ...
        const res = await axios.get(`https://generator2.hypernova.uz/api/ru/product/${id}`)
    ...
    const updateProduct = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`https://generator2.hypernova.uz/api/ru/update-product/${id}`, data, {
                headers: {
            ...
            const res = await axios.put(`https://generator2.hypernova.uz/api/ru/add-img/${id}`, formData, {
                headers: {
            ...
            addElement()
            await navigateTo(`/rus/card/${findProduct.value.category._id}`)
        } catch (err) {
            console.log(err)
        }
    }
    ...
    const addOffer = async (e) => {
        ...
        const res = await axios.put(`https://generator2.hypernova.uz/api/ru/offer/${id}`, data, {
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token.value}`,
            },
    ...
    const deleteOffer = async () => {
        const res = await axios.put(`https://generator2.hypernova.uz/api/ru/delete-offer/${id}`, data, {
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token.value}`,
            }
